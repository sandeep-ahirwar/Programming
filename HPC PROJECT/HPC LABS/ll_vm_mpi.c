...
int main (int argc, char *argv[]) {
    ...
    MPI_Scatter(a, n_per_proc, MPI_INT, a_pvt, n_per_proc, MPI_INT,
    MASTER, MPI_COMM_WORLD);
    MPI_Scatter(b, n_per_proc, MPI_INT, b_pvt, n_per_proc, MPI_INT,
    MASTER, MPI_COMM_WORLD);
    ...
    for(i=0;i<n_per_proc;i++){
        ...
        printf("Multiplication Done on %s\n", processor_name);
        printf("Product ~ %d\n", sum_pvt[i]);
    }
    MPI_Gather(sum_pvt, n_per_proc, MPI_INT, sum, n_per_proc, MPI_INT,
    MASTER, MPI_COMM_WORLD);
    if (rank == MASTER) {
